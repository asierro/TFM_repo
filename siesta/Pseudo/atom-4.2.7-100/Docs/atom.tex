...
\hrulefill
\par\vspace{3cm}
{\Large {\bf Version 4.2.0, 20 January 2017}}
\par\vspace{2cm}
\hrulefill
...
(Note that {\sc ATOM} now depends on the GridXC and xmlf90 libraries
for its correct compilation. Please follow the instructions in the
                                {\tt 000\_INSTALL} file.)
...
\item Second line:
	...
	\item Exchange-correlation type. Here, {\tt ca} stands for
          ...
          Support for libxc functionals (if compiled in in libGridXC)
          is available through the use of the special code {\tt xc},
          and a composite integer of the form 0XXX0YYY that follows
          later in the line. For example:
          \begin{verbatim}
...
\end{verbatim}
         ...
         where the code ``01010130'' packs the two codes ``101'' and
         ``130'' for the PBE functionals.
