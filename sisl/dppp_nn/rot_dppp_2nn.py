...
def rotate_phenyl(geom, which, angle):
    ...
    if which == 1:
        rotated = geom.rotate(angle - 28.67, coords[66] - coords[71], atoms=rot_phenyl1, origo=coords[71], only='xyz')
    elif which == 2:
        rotated = geom.rotate(angle + 28.23, coords[72] - coords[77], atoms=rot_phenyl2, origo=coords[77], only='xyz')
    elif which == 3:
        rotated = geom.rotate(angle - 28.28, coords[79] - coords[81], atoms=rot_phenyl3, origo=coords[81], only='xyz')
    else:
        rotated = geom.rotate(angle + 28.85, coords[54] - coords[84], atoms=rot_phenyl4, origo=coords[84], only='xyz')
    return rotated
...
for ia in rotating_dppp:
    ...
    idx_flat, xyz_flat = flat_dppp.close(ia, R=(0.1, maxR + 0.1), ret_xyz=True)
    ...
    for j, ja in enumerate(idx[1]):
        ...
        if ia in p24 and ja not in p24:
            if ia in phenyl2:
