...
def half_torsion1(alpha, geom):
    ...
    rot_geom = geom.rotate(alpha, geom_coords[17] - geom_coords[16], origin=geom_coords[16], only='xyz')
...
def symmetrize(geom):
    ...
    rotated = geom.rotate(sol, geom_coords[17] - geom_coords[16], origin=geom_coords[16], only='xyz')
    # print(["{:.13f}".format(i) for i in geom_coords[17] - geom_coords[16]])
    rot_coords = rotated.axyz()
    if rot_coords[5, 2] < rot_coords[3, 2]:
        return rotated.rotate(180, geom_coords[17] - geom_coords[16], origin=geom_coords[16], only='xyz'), sol+180
    return rotated, sol
...
def torsion_root(beta, geom):
    ...
    """
    geom_coords = geom.axyz()
    rot_geom = geom.rotate(beta, geom_coords[2] - geom_coords[17], atoms=rot_atoms, origin=geom_coords[17], only='xyz')
    rot_coords = rot_geom.axyz()
...
dnbp = dnbp.rotate(ang, rot_vec, origin=coords[16], only='xyz', rad=True)
...
for angle in range(0, 360, 5):
    rotated = dnbp.rotate(angle, coords[2] - coords[17], atoms=rot_atoms, origin=coords[17], only='xyz')
